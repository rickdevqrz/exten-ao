...
async function copyReport() {
  ...
  const contextLine =
    lastResult.context === "query"
      ? `Assunto: ${lastResult.title || ""}`
      : `URL: ${lastResult.url || ""}`;

  const report = [
    `Detector de Fake News (BR)`,
    contextLine,
    `Titulo: ${lastResult.title || ""}`,
    `Indice de suspeita: ${lastResult.score || 0}`,
    `Veredito: ${formatVerdict(lastResult.verdict) || "indisponivel"}`,
    `Confianca: ${
      Number.isFinite(lastResult.confidence) ? `${Math.round(lastResult.confidence * 100)}%` : "-"
    }`,
    `Nivel: ${getLevelMeta(lastResult.score || 0).level}/5 - ${
      getLevelMeta(lastResult.score || 0).label
    }`,
    `Motivos:`,
    ...(lastResult.reasons && lastResult.reasons.length > 0
      ? lastResult.reasons.map((reason) => `- ${reason}`)
      : ["- Nenhum motivo listado."]),
    `Fontes:`,
    ...(lastResult.sources && lastResult.sources.length > 0
      ? lastResult.sources.map((source) => {
          const title = source.title ? source.title.trim() : "";
          const link = source.url || source.domain || "";
